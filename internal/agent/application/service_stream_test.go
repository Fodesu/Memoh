...
func TestSendWSAgentEventWaitsForTerminalConsumerAfterExecutionCancellation(t *testing.T) {
	...
	if got.Type != native.EventAgentAbort || !got.HistoryCommitted {
...
func TestSendWSAgentEventBoundsTerminalDeliveryWithoutConsumer(t *testing.T) {
	...
	event := native.StreamEvent{Type: native.EventAgentEnd, HistoryCommitted: true}
	data, err := json.Marshal(event)
	if err != nil {
		t.Fatalf("marshal terminal event: %v", err)
	}
...
func TestPrepareRunConfigCarriesTerminalHookAuthority(t *testing.T) {
	...
	defer cancel(nil)
	validated := false
	authority := native.TerminalHookAuthority{
	...
	resolver := &Service{}
	got := resolver.prepareRunConfig(WithTerminalHookAuthority(context.Background(), authority), native.RunConfig{})
