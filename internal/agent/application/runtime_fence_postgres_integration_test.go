...
func TestPostgresValkeyMultiprocessCommandReconciliationChild(t *testing.T) {
	...
	if _, err := manager.HandleAgentEvent(runCtx, handle, native.StreamEvent{
		Type: native.EventUserInputRequest, ToolName: userinput.ToolNameAskUser,
...
func TestPostgresValkeyMultiprocessCrashReconcilesCommittedCommand(t *testing.T) {
	...
	payload, err := json.Marshal(application.UserInputResponseInput{Answers: []userinput.QuestionAnswer{ready.Answer}})
	if err != nil {
