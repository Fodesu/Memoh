...
func TestStoreMessagesUsesAtomicRoundForRuntimeFence(t *testing.T) {
	...
	persisted, err := resolver.storeMessages(ctx, ChatRequest{
		BotID:       storeRoundBotID,
		ThreadID:    sessionID,
		Query:       "hello",
		SessionType: "chat",
...
func TestStoreMessagesCommitsReplacementWithFencedRound(t *testing.T) {
	...
	_, err := resolver.storeMessages(ctx, ChatRequest{
		BotID: storeRoundBotID, ThreadID: sessionID, ReusePersistedUserMessage: true,
		PersistedUserMessageID: "request-old", SkipHistoryTurn: true,
	}, []ModelMessage{{Role: "assistant", Content: newTextContent("replacement")}}, "", storeRoundOptions{})
	if err != nil {
		t.Fatalf("storeMessages() error = %v", err)
	}
...
func TestStoreMessagesRejectsNonAtomicReplacementService(t *testing.T) {
	...
	_, err := resolver.storeMessages(ctx, ChatRequest{
		BotID: storeRoundBotID, ThreadID: "33333333-3333-3333-3333-333333333333",
