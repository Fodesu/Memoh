...
func (r *terminalHookTestRunner) Run(ctx context.Context, req hooks.Request, _ hooks.ToolRunner) (hooks.Result, error) {
	...
	r.completed <- req
	return hooks.Result{Decision: hooks.DecisionAllow}, r.terminalErr
}
...
func TestRunStreamTerminalHookTimeoutDoesNotConsumeTerminalDeliveryBudget(t *testing.T) {
	runner := newTerminalHookTestRunner(true)
	a := &Agent{
		hookService:                  runner,
		logger:                       slog.New(slog.DiscardHandler),
		terminalHookTimeout:          80 * time.Millisecond,
		terminalEventDeliveryTimeout: 30 * time.Millisecond,
	}
	events := a.Stream(context.Background(), terminalHookRunConfig(&terminalHookTestProvider{}, TerminalHookAuthority{}))
	if event := <-events; event.Type != EventAgentStart {
		t.Fatalf("first event = %q, want %q", event.Type, EventAgentStart)
	}
	...
	case err := <-runner.canceled:
		if !errors.Is(err, context.DeadlineExceeded) {
			t.Fatalf("terminal hook cancellation = %v, want deadline exceeded", err)
		}
	case <-time.After(time.Second):
		t.Fatal("terminal hook did not respect its total timeout")
	}

	// The delivery timeout is intentionally shorter than the hook timeout. A
	// delivery deadline created before the hook would already be exhausted.
	time.Sleep(10 * time.Millisecond)
	if event := terminalEventFromStream(t, events); event.Type != EventAgentEnd {
		t.Fatalf("terminal event = %q, want %q", event.Type, EventAgentEnd)
	}
}

func TestRunStreamTerminalHookFailureStillDeliversTerminalOnce(t *testing.T) {
	runner := newTerminalHookTestRunner(false)
	runner.terminalErr = errors.New("hook failed")
	a := &Agent{
		hookService:                  runner,
		logger:                       slog.New(slog.DiscardHandler),
		terminalHookTimeout:          50 * time.Millisecond,
		terminalEventDeliveryTimeout: 50 * time.Millisecond,
	}

	terminalCount := 0
	for event := range a.Stream(context.Background(), terminalHookRunConfig(&terminalHookTestProvider{}, TerminalHookAuthority{})) {
		if event.Type == EventAgentEnd || event.Type == EventAgentAbort {
			terminalCount++
		}
	}
