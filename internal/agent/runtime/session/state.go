...
func uiMessageIndex(messages []chatview.UIMessage, incoming chatview.UIMessage) int {
...
func preserveTerminalDecision(existing, incoming chatview.UIMessage) chatview.UIMessage {
	if existing.Type != chatview.UIMessageTool || incoming.Type != chatview.UIMessageTool {
		return incoming
	}
...
func resolvedUIMessageUpserts(run *CurrentRunView, requested []chatview.UIMessage) []chatview.UIMessage {
	if run == nil || len(requested) == 0 {
		return nil
	}
	resolved := make([]chatview.UIMessage, 0, len(requested))
