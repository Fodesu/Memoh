	...
	if _, changed, updateErr := backend.Update(context.Background(), key, func(Snapshot, bool) (Snapshot, bool, error) {
		return Snapshot{
			...
			CurrentRunView: &CurrentRunView{
				...
				Messages: []chatview.UIMessage{{ID: 1, Type: chatview.UIMessageTool, Input: input, Progress: []any{[]any{}}}},
			},
		}, true, nil
...
func runRedisPubSubFidelityContract(t *testing.T, redisURL string) {
	...
	if err := backend.Publish(context.Background(), Event{
		...
		Delta: &RuntimeDelta{MessageUpserts: []chatview.UIMessage{{
			ID: 1, Type: chatview.UIMessageTool, Input: input, Progress: []any{[]any{}},
		}}},
	}); err != nil {
...
func runRedisActiveReaperContract(t *testing.T, redisURL string) {
	...
	if err := manager.StartRun(context.Background(), testBotID, "session-active-reaper", "stream-active-reaper", make(chan struct{}, 1), func() {}, make(chan turn.InjectMessage, 1)); err != nil {
...
func runRedisStartRunRetryClockContract(t *testing.T, redisURL string) {
	...
	if err := manager.StartRun(context.Background(), testBotID, "session-start-retry-clock", "stream-start-retry-clock", make(chan struct{}, 1), func() {}, make(chan turn.InjectMessage, 1)); err != nil {
...
func runRedisNormalizedMessageAppendContract(t *testing.T, redisURL string) {
	...
	if _, err := manager.HandleAgentEvent(context.Background(), handle, native.StreamEvent{Type: native.EventTextDelta, Delta: "hello"}); err != nil {
	...
	if _, err := manager.HandleAgentEvent(context.Background(), handle, native.StreamEvent{Type: native.EventTextDelta, Delta: " world"}); err != nil {
