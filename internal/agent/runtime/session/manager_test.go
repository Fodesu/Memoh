...
func runRuntimeManagerProjectsDecisionsContract(t *testing.T, suite runtimeBackendContractSuite) {
	...
	requests := []native.StreamEvent{
		{Type: native.EventUserInputRequest, ToolName: "ask_user", ToolCallID: "call-input-submit", UserInputID: "input-submit", Status: "pending"},
		{Type: native.EventUserInputRequest, ToolName: "ask_user", ToolCallID: "call-input-cancel", UserInputID: "input-cancel", Status: "pending"},
		{Type: native.EventUserInputRequest, ToolName: "ask_user", ToolCallID: "call-input-failed", UserInputID: "input-failed", Status: "pending"},
		{Type: native.EventToolApprovalRequest, ToolName: "exec", ToolCallID: "call-approval-approve", ApprovalID: "approval-approve", Status: "pending"},
		{Type: native.EventToolApprovalRequest, ToolName: "exec", ToolCallID: "call-approval-reject", ApprovalID: "approval-reject", Status: "pending"},
		{Type: native.EventToolApprovalRequest, ToolName: "exec", ToolCallID: "call-approval-failed", ApprovalID: "approval-failed", Status: "pending"},
	}
...
func decisionMessageState(message chatview.UIMessage, targetID string) (string, bool) {
	if message.UserInput != nil && message.UserInput.UserInputID == targetID {
		return message.UserInput.Status, message.UserInput.CanRespond
	}
	if message.Approval != nil && message.Approval.ApprovalID == targetID {
...
func runRuntimeManagerKeepsCommittedDecisionsTerminalContract(t *testing.T, suite runtimeBackendContractSuite) {
	...
	for _, decision := range []struct {
		...
		{
			request: native.StreamEvent{
				Type: native.EventUserInputRequest, ToolName: "ask_user", ToolCallID: "call-stale-input",
		...
		{
			request: native.StreamEvent{
				Type: native.EventToolApprovalRequest, ToolName: "exec", ToolCallID: "call-stale-approval",
		...
		if _, err := owner.HandleAgentEvent(context.Background(), handle, native.StreamEvent{
			Type: native.EventToolCallEnd, ToolName: decision.request.ToolName,
...
func runRuntimeManagerOrdersDecisionProjectionBeforeContinuationContract(t *testing.T, suite runtimeBackendContractSuite) {
	...
	handle := requireRunHandle(t, owner, testBotID, testSessionID, testStreamID)
	request := native.StreamEvent{
		Type: native.EventUserInputRequest, ToolName: "ask_user", ToolCallID: "call-decision-order",
	...
	go func() {
		...
		_, continuationErr := owner.HandleAgentEvent(context.Background(), handle, native.StreamEvent{
			Type: native.EventToolCallEnd, ToolName: request.ToolName,
...
func runRuntimeManagerRetriesDecisionProjectionBeforeContinuationContract(t *testing.T, suite runtimeBackendContractSuite) {
	...
	handle := requireRunHandle(t, manager, testBotID, testSessionID, testStreamID)
	request := native.StreamEvent{
		Type: native.EventUserInputRequest, ToolName: "ask_user", ToolCallID: "call-decision-projection-retry",
...
func runRuntimeManagerReplaysCommittedDecisionProjectionsContract(t *testing.T, suite distributedRuntimeBackendContractSuite) {
	...
	handle := requireRunHandle(t, owner, testBotID, testSessionID, testStreamID)
	if _, err := owner.HandleAgentEvent(context.Background(), handle, native.StreamEvent{
		Type: native.EventUserInputRequest, ToolName: "ask_user", ToolCallID: "call-projection-replay",
	...
	if _, err := owner.HandleAgentEvent(context.Background(), handle, native.StreamEvent{
		Type: native.EventToolApprovalRequest, ToolName: "exec", ToolCallID: "call-approval-projection-replay",
