...
func TestMemoryBackendPreservesDynamicJSONShapes(t *testing.T) {
	...
	if err := backend.Publish(context.Background(), Event{
		...
		Delta: &RuntimeDelta{MessageUpserts: []chatview.UIMessage{{ID: 1, Type: chatview.UIMessageTool, Input: input, Progress: []any{[]any{}}}}},
	}); err != nil {
...
func assertDynamicJSONFidelity(t *testing.T, message chatview.UIMessage) {
	t.Helper()
