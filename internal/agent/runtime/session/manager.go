...
func (m *Manager) FinalizeAgentEvent(ctx context.Context, handle RunHandle, event native.StreamEvent, canonicalReady bool, finalizationError string) ([]chatview.UIMessage, error) {
...
func (m *Manager) finalizeRunState(ctx context.Context, handle RunHandle, outcome runFinalization) (bool, error) {
	...
	_, changed, err := m.releaseActiveAndPublish(ctx, handle, func(snapshot Snapshot, now time.Time) (Snapshot, bool, error) {
		...
		delta.MessageUpserts = append([]chatview.UIMessage(nil), outcome.Messages...)
		return delta
	})
	return changed, err
}
