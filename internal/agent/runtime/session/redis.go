...
func (b *RedisBackend) AppendActiveRunMessage(ctx context.Context, key Key, ref StreamRef, messageAppend RuntimeMessageAppend) (RuntimeRevision, bool, error) {
	...
	if messageAppend.Type != chatview.UIMessageText && messageAppend.Type != chatview.UIMessageReasoning {
		return RuntimeRevision{}, false, nil
	}
	...
	contentField := redisMessageContentField(ref.Generation, chatview.UIMessage{
		ID:   messageAppend.ID,
...
func prepareRedisSnapshot(snapshot Snapshot) (preparedRedisSnapshot, error) {
	...
	if snapshot.CurrentRunView != nil {
		...
		run.Messages = make([]chatview.UIMessage, len(snapshot.CurrentRunView.Messages))
		copy(run.Messages, snapshot.CurrentRunView.Messages)
		for index := range run.Messages {
			message := &run.Messages[index]
			if message.Type != chatview.UIMessageText && message.Type != chatview.UIMessageReasoning {
				continue
			}
...
func redisMessageContentField(generation string, message chatview.UIMessage) string {
...
func decodeRedisSnapshot(data []byte) (Snapshot, error) {
	...
	if snapshot.CurrentRunView != nil && snapshot.CurrentRunView.Messages == nil {
		...
		snapshot.CurrentRunView.Messages = []chatview.UIMessage{}
	}
	return snapshot, nil
}
