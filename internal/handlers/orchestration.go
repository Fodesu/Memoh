...
func (h *OrchestrationHandler) StartRun(c echo.Context) error {
	...
	botID, err := effectiveStartRunBotID(req)
	if err != nil {
		return echo.NewHTTPError(http.StatusBadRequest, err.Error())
	}
	if botID != "" {
...
func effectiveStartRunBotID(req orchestration.StartRunRequest) (string, error) {
	botID := strings.TrimSpace(req.BotID)
	metadataBotID := strings.TrimSpace(stringMapValue(req.SourceMetadata, "bot_id"))
	if botID != "" && metadataBotID != "" && botID != metadataBotID {
		return "", errors.New("bot_id does not match source_metadata.bot_id")
	}
	if botID != "" {
		return botID, nil
	}
	return metadataBotID, nil
}
