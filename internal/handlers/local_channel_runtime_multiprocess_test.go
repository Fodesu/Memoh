...
func (s *runtimeSmokeState) startRun(manager *sessionruntime.Manager, streamID, extraText string, processDone <-chan struct{}) error {
	...
	injectCh := make(chan turn.InjectMessage, 4)
	cancelRun := func() {
		runCancel()
		s.mu.Lock()
		s.canceled = true
	...
	handle, err := manager.StartRunWithOptions(runCtx, sessionruntime.RunStartOptions{
		...
		AbortCh: abortCh, Cancel: cancelRun, InjectCh: injectCh,
	})
	if err != nil {
		runCancel()
		s.mu.Lock()
...
func TestLocalChannelRuntimeMultiprocessServer(t *testing.T) {
	...
	state := &runtimeSmokeState{}
	resolver := &runtimeSmokeResolver{Service: &application.Service{}, state: state}
...
func TestLocalChannelRuntimeMultiprocessSmoke(t *testing.T) {
	...
	if !runtimeSnapshotHasMessage(*snapshotEvent.Snapshot, chatview.UIMessageText, "", "Owner B initial output.") {
	...
	readRuntimeSmokeEventUntil(t, clientA, func(event sessionruntime.Event) bool {
		return event.Type == sessionruntime.EventRuntimeDelta && runtimeDeltaHasMessageAppend(event, chatview.UIMessageText, " Cross-process delta.")
	})
	...
	if !runtimeSnapshotHasMessage(*rehydrated.Snapshot, chatview.UIMessageText, "", "Owner B restarted output.") {
	...
	readRuntimeSmokeEventUntil(t, clientAfterRestart, func(event sessionruntime.Event) bool {
		return event.Type == sessionruntime.EventRuntimeDelta && runtimeDeltaHasMessageAppend(event, chatview.UIMessageText, " Delta after server A restart.")
	})
