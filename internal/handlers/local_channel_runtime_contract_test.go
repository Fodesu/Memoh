...
func richActiveRunAgentContractScript() []native.StreamEvent {
...
func richActiveRunActiveAgentContractScript() []native.StreamEvent {
	events := richActiveRunAgentContractScript()
...
func interruptedRunAgentContractScript() []native.StreamEvent {
	return []native.StreamEvent{
		{Type: native.EventAgentStart},
		{Type: native.EventTextDelta, Delta: "partial output"},
		{Type: native.EventError, Error: "runtime interrupted"},
...
func TestLocalChannelRuntimeManagedStreamDoesNotWriteLegacyFrames(t *testing.T) {
	...
	if !runtimeSnapshotHasMessage(snapshot, chatview.UIMessageText, "", "runtime only") {
...
func TestLocalChannelStartWSStreamDrainsBufferedRuntimeEventsBeforeFinish(t *testing.T) {
	...
	var text string
	for _, block := range snapshot.CurrentRunView.Messages {
		if block.Type == chatview.UIMessageText {
			text = block.Content
			break
		}
	}
...
func TestLocalChannelStartWSStreamFinalizesCommittedPartialAbortAfterExecutionCancellation(t *testing.T) {
	...
	waitHandlerRuntimeSnapshot(t, manager, func(snapshot sessionruntime.Snapshot) bool {
		return runtimeSnapshotHasMessage(snapshot, chatview.UIMessageText, "", "partial answer")
	})
	...
	if !runtimeSnapshotHasMessage(snapshot, chatview.UIMessageText, "", "partial answer") {
...
func TestLocalChannelRuntimeLegacyTerminalUsesFinalizationContext(t *testing.T) {
	...
	go func() {
		...
		forwardDone <- handler.forwardRuntimeWSStreamEvents(
			...
			eventCh,
			nil,
			func(ctx context.Context, _ native.StreamEvent) {
				legacyCtx <- ctx
				<-releaseLegacy
			},
		)
...
func TestLocalChannelRuntimeTerminalCommitsBeforeLegacyDelivery(t *testing.T) {
	...
	go func() {
		...
		forwardDone <- handler.forwardRuntimeWSStreamEvents(
			...
			eventCh,
			nil,
			func(context.Context, native.StreamEvent) {
				close(legacyStarted)
				<-releaseLegacy
			},
		)
...
func TestLocalChannelRuntimeWaitsForAssetLinkBeforeCanonicalTerminal(t *testing.T) {
	...
	if err := manager.StartRun(context.Background(), runtimeContractBotID, runtimeContractSessionID, "stream-asset-order", make(chan struct{}, 1), func() {}, make(chan turn.InjectMessage, 1)); err != nil {
		t.Fatalf("start runtime run: %v", err)
	}
	t.Cleanup(func() { _ = manager.Close() })
...
func TestLocalChannelRuntimeRetriesCompleteTerminalOutcome(t *testing.T) {
	...
	if err := manager.StartRun(context.Background(), runtimeContractBotID, runtimeContractSessionID, "stream-terminal-retry", make(chan struct{}, 1), func() {}, make(chan turn.InjectMessage, 1)); err != nil {
		t.Fatalf("start runtime run: %v", err)
	}
	t.Cleanup(func() { _ = manager.Close() })
...
func TestLocalChannelRuntimeUpdateFailureCancelsAndErrorsRun(t *testing.T) {
	...
	handler.startWSStream(
		...
		func(ctx context.Context, eventCh chan<- application.WSStreamEvent, _ <-chan struct{}, _ <-chan turn.InjectMessage) error {
			...
			eventCh <- rawRuntimeContractEvent(t, native.StreamEvent{Type: native.EventTextDelta, Delta: "lost delta"})
			<-ctx.Done()
...
func TestLocalChannelRuntimeForwarderBatchesAdjacentTextDeltas(t *testing.T) {
	...
	if err := manager.StartRun(context.Background(), runtimeContractBotID, runtimeContractSessionID, runtimeContractStreamID, make(chan struct{}, 1), func() {}, make(chan turn.InjectMessage, 1)); err != nil {
		t.Fatalf("start runtime run: %v", err)
	}
	...
	if !runtimeSnapshotHasMessage(snapshot, chatview.UIMessageText, "", strings.Repeat("x", 100)) {
...
func TestLocalChannelPersistentRuntimePublishFailureReconcilesWithoutCancelingRun(t *testing.T) {
	...
	if !runtimeSnapshotHasMessage(snapshot, chatview.UIMessageText, "", "committed delta") {
...
func TestLocalChannelHandleWebSocketRuntimeSubscribeReturnsSnapshotAndAgentDeltas(t *testing.T) {
	...
	if !runtimeSnapshotHasMessage(*snapshot, chatview.UIMessageReasoning, "", "I need to inspect the workspace.") {
		t.Fatalf("snapshot messages = %#v, want reasoning block", snapshot.CurrentRunView.Messages)
	}
	if !runtimeSnapshotHasMessage(*snapshot, chatview.UIMessageText, "", "I will check the current state.") {
	...
	deltaEvent := readRuntimeContractEventUntil(t, client, func(event sessionruntime.Event) bool {
		return event.Type == sessionruntime.EventRuntimeDelta &&
			...
			runtimeDeltaHasMessageAppend(event, chatview.UIMessageText, " Late delta.")
	})
...
func TestLocalChannelHandleWebSocketSecondClientAttachesActiveRunAndReceivesDeltas(t *testing.T) {
	...
	for _, client := range []*websocket.Conn{first, second} {
		...
		if snapshotEvent.Snapshot == nil || !runtimeSnapshotHasMessage(*snapshotEvent.Snapshot, chatview.UIMessageText, "", "Initial output.") {
	...
	if _, err := manager.HandleAgentEvent(context.Background(), requireHandlerRunHandle(t, manager, runtimeContractBotID, runtimeContractSessionID, runtimeContractStreamID), native.StreamEvent{
		Type:  native.EventTextDelta,
	...
	for _, client := range []*websocket.Conn{first, second} {
		deltaEvent := readRuntimeContractEventUntil(t, client, func(event sessionruntime.Event) bool {
			return event.Type == sessionruntime.EventRuntimeDelta &&
				runtimeDeltaHasMessageAppend(event, chatview.UIMessageText, " Delta for both clients.")
		})
		if deltaEvent.StreamID != runtimeContractStreamID {
			t.Fatalf("delta stream id = %q, want %q", deltaEvent.StreamID, runtimeContractStreamID)
		}
...
func TestLocalChannelHandleWebSocketOrdinarySendPublishesRequestTurnToAllSubscribers(t *testing.T) {
	...
	resolver := &scriptedOrdinaryResolver{
		Service: application.NewService(slog.Default(), nil, nil, nil, nil, nil, nil, time.UTC, time.Second),
		started: make(chan application.ChatRequest, 1),
		release: releaseRun,
	}
	handler := runtimeContractLocalChannelHandler(manager)
...
func TestLocalChannelDistributedAdmissionCarriesPersistenceFence(t *testing.T) {
	...
	resolver := &scriptedOrdinaryResolver{
		...
		authority:    make(chan native.TerminalHookAuthority, 1),
	}
	handler := runtimeContractLocalChannelHandler(manager)
	handler.SetAgentService(resolver)
	client := openLocalChannelTestWS(t, handler, runtimeContractBotID, runtimeContractUserID)
	...
	var authority native.TerminalHookAuthority
	select {
...
func TestLocalChannelHandleWebSocketCanAbortBlockedReplacementAdmission(t *testing.T) {
	...
	handler := runtimeContractLocalChannelHandler(manager)
	handler.agentService = &blockingReplacementResolver{
...
func TestLocalChannelHandleWebSocketFinalizesCanceledRunWithoutTerminalEvent(t *testing.T) {
	...
	started := make(chan struct{})
	handler := runtimeContractLocalChannelHandler(manager)
	handler.agentService = &cancelBlockingReplacementResolver{
		scriptedReplacementResolver: newScriptedReplacementResolver(sessionruntime.RunOperationRetry, nil),
		started:                     started,
	}
	client := openLocalChannelTestWS(t, handler, runtimeContractBotID, runtimeContractUserID)
...
func TestLocalChannelHandleWebSocketKeepsMultipleSessionRuntimeSubscriptions(t *testing.T) {
	...
	for _, sessionID := range []string{runtimeContractSessionID, runtimeContractOtherSessionID} {
		...
		if _, err := manager.HandleAgentEvent(context.Background(), requireHandlerRunHandle(t, manager, runtimeContractBotID, sessionID, streams[sessionID]), native.StreamEvent{
			Type:  native.EventTextDelta,
			Delta: text,
		}); err != nil {
		...
		deltaEvent := readRuntimeContractEventUntil(t, client, func(event sessionruntime.Event) bool {
			return event.Type == sessionruntime.EventRuntimeDelta &&
				...
				runtimeDeltaHasMessageAppend(event, chatview.UIMessageText, text)
		})
...
func TestLocalChannelHandleWebSocketRuntimeSubscribeDoesNotReplaySnapshotDeltas(t *testing.T) {
	...
	if snapshotEvent.Snapshot == nil || !runtimeSnapshotHasMessage(*snapshotEvent.Snapshot, chatview.UIMessageText, "", "Gap delta.") {
...
func TestLocalChannelHandleWebSocketActiveRunResponsesAreSidebandCommands(t *testing.T) {
	...
	for _, event := range []native.StreamEvent{
		...
		{Type: native.EventUserInputRequest, ToolName: "ask_user", ToolCallID: "call-ask", UserInputID: "input-1", Status: "pending"},
	} {
		if _, err := manager.HandleAgentEvent(context.Background(), requireHandlerRunHandle(t, manager, runtimeContractBotID, runtimeContractSessionID, runtimeContractStreamID), event); err != nil {
	...
	resolver := &sidebandResponseResolver{
		Service:   &application.Service{},
		approvals: make(chan application.ToolApprovalResponseInput, 1),
		inputs:    make(chan application.UserInputResponseInput, 1),
	}
	handler := runtimeContractLocalChannelHandler(manager)
	handler.SetAgentService(resolver)
	client := openLocalChannelTestWS(t, handler, runtimeContractBotID, runtimeContractUserID)
...
func TestLocalChannelSidebandResponseCanFinishRunWithoutDeferredStream(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "approval",
			target: native.StreamEvent{
				Type: native.EventToolApprovalRequest, ToolName: "exec", ToolCallID: "call-approval-terminal",
		...
		},
		{
			name: "user_input",
			target: native.StreamEvent{
				Type: native.EventUserInputRequest, ToolName: "ask_user", ToolCallID: "call-input-terminal",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := manager.StartRun(context.Background(), runtimeContractBotID, runtimeContractSessionID, runtimeContractStreamID,
				make(chan struct{}, 1), func() {}, make(chan turn.InjectMessage, 1)); err != nil {
				t.Fatalf("start runtime run: %v", err)
			}
			handle := requireHandlerRunHandle(t, manager, runtimeContractBotID, runtimeContractSessionID, runtimeContractStreamID)
			...
			resolver := &sidebandResponseResolver{
				Service:       &application.Service{},
				approvals:     make(chan application.ToolApprovalResponseInput, 2),
				inputs:        make(chan application.UserInputResponseInput, 2),
...
func TestLocalChannelHandleWebSocketRoutesActiveResponseToRemoteOwner(t *testing.T) {
	...
	if err := owner.StartRun(runtimefence.WithContext(context.Background(), ownerFence), runtimeContractBotID, runtimeContractSessionID, runtimeContractStreamID, make(chan struct{}, 1), func() {}, make(chan turn.InjectMessage, 1)); err != nil {
		t.Fatalf("start owner run: %v", err)
	}
	if _, err := owner.HandleAgentEvent(context.Background(), requireHandlerRunHandle(t, owner, runtimeContractBotID, runtimeContractSessionID, runtimeContractStreamID), native.StreamEvent{
		Type:       native.EventToolApprovalRequest,
		ToolName:   "exec",
		ToolCallID: "call-approval",
	...
	ownerHandler.SetAgentService(ownerResolver)
	...
	case input := <-ownerResolver.approvals:
		if input.ApprovalID != "approval-remote" || input.ExplicitID != "approval-remote" || input.BotID != runtimeContractBotID || input.ThreadID != runtimeContractSessionID || !input.ResolveOnly {
...
func TestLocalChannelHandleWebSocketRuntimeUnsubscribeStopsSessionEvents(t *testing.T) {
	...
	for sessionID, streamID := range map[string]string{
		...
		if err := manager.StartRun(context.Background(), runtimeContractBotID, sessionID, streamID, make(chan struct{}, 1), func() {}, make(chan turn.InjectMessage, 1)); err != nil {
	...
	if _, err := manager.HandleAgentEvent(context.Background(), requireHandlerRunHandle(t, manager, runtimeContractBotID, runtimeContractSessionID, runtimeContractStreamID), native.StreamEvent{Type: native.EventTextDelta, Delta: "must not arrive"}); err != nil {
...
func TestLocalChannelHandleWebSocketRuntimeResubscribeOrdersOldFramesBeforeCheckpoint(t *testing.T) {
	...
	handle := requireHandlerRunHandle(t, manager, runtimeContractBotID, runtimeContractSessionID, runtimeContractStreamID)
	if _, err := manager.HandleAgentEvent(context.Background(), handle, native.StreamEvent{Type: native.EventTextDelta, Delta: "before checkpoint"}); err != nil {
	...
	if _, err := manager.HandleAgentEvent(context.Background(), handle, native.StreamEvent{Type: native.EventTextDelta, Delta: " after checkpoint"}); err != nil {
...
func TestLocalChannelHandleWebSocketRuntimeUnsubscribeAfterAccessRevocation(t *testing.T) {
	...
	if _, err := manager.HandleAgentEvent(context.Background(), requireHandlerRunHandle(t, manager, runtimeContractBotID, runtimeContractSessionID, runtimeContractStreamID), native.StreamEvent{Type: native.EventTextDelta, Delta: "must not arrive after revoked unsubscribe"}); err != nil {
		t.Fatalf("publish unsubscribed runtime delta: %v", err)
	}
	assertNoRuntimeContractEvent(t, client, 200*time.Millisecond, func(event sessionruntime.Event) bool {
		return event.SessionID == runtimeContractSessionID && event.Type == sessionruntime.EventRuntimeDelta
...
type scriptedOrdinaryResolver struct {
	*application.Service
	started      chan application.ChatRequest
	release      <-chan struct{}
	fence        runtimefence.Fence
	prepareFence chan runtimefence.Fence
	streamFence  chan runtimefence.Fence
	authority    chan native.TerminalHookAuthority
}
...
type sidebandResponseResolver struct {
	*application.Service
	approvals      chan application.ToolApprovalResponseInput
	inputs         chan application.UserInputResponseInput
...
func (r *deferredResponseResolver) PrepareToolApprovalResponse(context.Context, application.ToolApprovalResponseInput) (runtimefence.PreservedDecision, error) {
	r.stages <- deferredResponseStage{name: "prepare"}
	return r.preserved, r.prepareErr
}

func (r *deferredResponseResolver) PrepareUserInputResponse(context.Context, application.UserInputResponseInput) (runtimefence.PreservedDecision, error) {
	r.stages <- deferredResponseStage{name: "prepare"}
	return r.preserved, r.prepareErr
}
...
func (r *scriptedOrdinaryResolver) AllocateRuntimePersistenceFence(ctx context.Context, botID, sessionID string) (runtimefence.Fence, error) {
	...
	return r.Service.AllocateRuntimePersistenceFence(ctx, botID, sessionID)
}
...
func (r *scriptedOrdinaryResolver) ActivateRuntimePersistenceFenceWithOptions(ctx context.Context, fence runtimefence.Fence, options runtimefence.ActivationOptions) error {
	...
	return r.Service.ActivateRuntimePersistenceFenceWithOptions(ctx, fence, options)
}

func (r *scriptedOrdinaryResolver) PrepareUserMessageWS(ctx context.Context, req application.ChatRequest) (application.ChatRequest, error) {
...
func (r *scriptedOrdinaryResolver) StreamChatWS(ctx context.Context, req application.ChatRequest, _ chan<- application.WSStreamEvent, _ <-chan struct{}) error {
	...
	if r.authority != nil {
		r.authority <- application.TerminalHookAuthorityFromContext(ctx)
	}
	select {
	case r.started <- req:
	case <-ctx.Done():
...
func (r *blockingReplacementResolver) ValidatePreparedReplacementWS(ctx context.Context, _ application.PreparedReplacementWS) error {
...
func newScriptedReplacementResolver(kind string, events []application.WSStreamEvent) *scriptedReplacementResolver {
	...
	resolver := application.NewService(slog.Default(), nil, nil, messages, nil, nil, nil, time.UTC, time.Second)
...
func runtimeSnapshotHasMessage(snapshot sessionruntime.Snapshot, kind chatview.UIMessageType, toolCallID, content string) bool {
	if snapshot.CurrentRunView == nil {
		return false
	}
...
func runtimeDeltaHasMessageAppend(event sessionruntime.Event, kind chatview.UIMessageType, content string) bool {
	if event.Delta == nil {
		return false
	}
