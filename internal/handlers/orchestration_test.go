...
func TestOrchestrationRegisterSkipsRoutesWithoutService(t *testing.T) {
	e := echo.New()
	...
	if len(e.Routes()) != 0 {
		t.Fatalf("Register() routes = %d, want 0 without service", len(e.Routes()))
	}
}

func TestOrchestrationRegisterIncludesCheckpointCreateRoute(t *testing.T) {
	e := echo.New()
	handler := &OrchestrationHandler{service: newNoopOrchestrationService()}
...
func TestOrchestrationResolveCheckpointPassesResolution(t *testing.T) {
	e, rec, req := newAuthedEcho(http.MethodPost, "/orchestration/checkpoints/checkpoint-1/resolve", `{"option":"approve","idempotency_key":"resolve-1","metadata":{"approved":true}}`)
	var called bool
	svc := newNoopOrchestrationService()
	svc.resolveCheckpoint = func(_ context.Context, _ orchestration.ControlIdentity, checkpointID string, req orchestration.CheckpointResolution) (*orchestration.ResolveCheckpointResult, error) {
		...
		if req.Option != "approve" || req.IdempotencyKey != "resolve-1" {
			t.Fatalf("req = %+v", req)
		}
...
func TestOrchestrationResolveCheckpointMapsErrorContract(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			e, rec, req := newAuthedEcho(http.MethodPost, "/orchestration/checkpoints/checkpoint-1/resolve", `{"option":"approve","idempotency_key":"resolve-1"}`)
			svc := newNoopOrchestrationService()
