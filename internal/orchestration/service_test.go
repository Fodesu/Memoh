...
func TestRootWorkerProfileUsesDefaultUnlessBuiltinFixtureIsExplicit(t *testing.T) {
	t.Parallel()

	if got := rootWorkerProfile(nil, nil); got != DefaultWorkerProfile {
		t.Fatalf("rootWorkerProfile(nil) = %q, want %q", got, DefaultWorkerProfile)
	}
	if got := rootWorkerProfile(nil, map[string]any{"bot_id": "bot-1"}); got != DefaultWorkerProfile {
		t.Fatalf("rootWorkerProfile(bot) = %q, want %q", got, DefaultWorkerProfile)
	}
...
func TestNormalizeWorkerProfilesLetsDefaultWorkerClaimBuiltinEcho(t *testing.T) {
	...
	profiles := normalizeWorkerProfiles([]string{DefaultWorkerProfile})
	if !containsStringForTest(profiles, DefaultWorkerProfile) {
		t.Fatalf("profiles = %#v, missing %q", profiles, DefaultWorkerProfile)
	}
...
func TestNormalizeCheckpointResolutionUsesSelectedOption(t *testing.T) {
	t.Parallel()
	...
	checkpoint := sqlc.OrchestrationHumanCheckpoint{
		...
		Options:       rawOptions,
		DefaultAction: marshalJSON(nil),
	}

	resolution, hash, err := normalizeCheckpointResolution(checkpoint, CheckpointResolution{
		Option:         "freeform",
		Response:       "continue with cached state",
		IdempotencyKey: "idem-2",
		Metadata:       nil,
	})
	if err != nil {
		t.Fatalf("normalizeCheckpointResolution(freeform) error = %v", err)
	}

	if resolution.Option != "freeform" {
		t.Fatalf("normalized option = %q, want freeform", resolution.Option)
	}
	if resolution.Response != "continue with cached state" {
		t.Fatalf("normalized response = %q", resolution.Response)
	}
...
func TestValidateCheckpointDefinitionRejectsDuplicateOptions(t *testing.T) {
	t.Parallel()
...
func TestValidateCheckpointDefinitionReturnsNormalizedValues(t *testing.T) {
	...
	if defaultAction.Option != "approve" {
		t.Fatalf("normalized default_action option = %q, want approve", defaultAction.Option)
	}
}
...
func TestDecodePlannedChildTasksNormalizesMinimalPlan(t *testing.T) {
	...
	plans := normalizeReplanMidTasks(decodePlannedChildTasks(map[string]any{
		"child_tasks": []any{
			map[string]any{
				"id":             "first",
				"goal":           " first child ",
				"worker_profile": " " + DefaultWorkerProfile + " ",
				"priority":       float64(2),
	...
	if len(plans) != 2 {
		t.Fatalf("decodePlannedChildTasks() len = %d, want 2 (empty-goal item is dropped)", len(plans))
	}
	...
	if plans[0].Role != TaskRoleMid {
		t.Fatalf("first child role = %q, want %q", plans[0].Role, TaskRoleMid)
	}
	if plans[0].WorkerProfile != DefaultWorkerProfile {
		t.Fatalf("first child worker_profile = %q, want %q", plans[0].WorkerProfile, DefaultWorkerProfile)
	}
	...
	if plans[1].Role != TaskRoleMid {
		t.Fatalf("second child role = %q, want %q", plans[1].Role, TaskRoleMid)
	}
	if plans[1].Alias == "" {
		t.Fatalf("second child alias = empty, want auto-assigned mid alias")
	}
	if plans[1].WorkerProfile != DefaultWorkerProfile {
		t.Fatalf("second child worker_profile = %q, want %q", plans[1].WorkerProfile, DefaultWorkerProfile)
	}
	...
	if err := validateReplanMidTasks(plans); err != nil {
		t.Fatalf("validateReplanMidTasks(normalized) error = %v", err)
	}
}

func TestValidatePlannedChildTasksRequiresSingleFinalTask(t *testing.T) {
	t.Parallel()

	plans := []plannedChildTask{
		{Alias: "first", Role: TaskRoleFinal, Goal: "first final"},
		{Alias: "second", Role: TaskRoleFinal, Goal: "second final"},
	}
	if err := validatePlannedChildTasks(plans); !errors.Is(err, ErrPlanningIntentInvalid) {
		t.Fatalf("validatePlannedChildTasks(two finals) error = %v, want %v", err, ErrPlanningIntentInvalid)
	}
}
...
func TestNormalizeCheckpointResolutionCanonicalizesChoiceResponse(t *testing.T) {
	t.Parallel()
	...
	if a.Response != "" || b.Response != "" {
		t.Fatalf("normalized choice response = %q / %q, want empty", a.Response, b.Response)
	}
	if hashA != hashB {
...
func TestFilterResultsByTask(t *testing.T) {
	t.Parallel()

	base := time.Date(2026, 4, 24, 10, 0, 0, 0, time.UTC)
	items := []TaskResult{
		{ID: "r1", TaskID: "task-1", CreatedAt: base},
		{ID: "r2", TaskID: "task-2", CreatedAt: base.Add(time.Second)},
		{ID: "r3", TaskID: "task-1", CreatedAt: base.Add(2 * time.Second)},
	}

	filtered := filterResults(items, "task-1")
	if len(filtered) != 2 || filtered[0].ID != "r1" || filtered[1].ID != "r3" {
		t.Fatalf("filterResults(task-1) = %+v, want r1,r3", filtered)
	}

	page, after, err := paginateResults(items, "", 2, 12, filterHash([]string{"task-1"}))
	if err != nil {
		t.Fatalf("paginateResults(page1) error = %v", err)
	}
	if len(page) != 2 || after == "" {
		t.Fatalf("paginateResults(page1) = %+v, after=%q", page, after)
	}
	page, after, err = paginateResults(items, after, 2, 12, filterHash([]string{"task-1"}))
	if err != nil {
		t.Fatalf("paginateResults(page2) error = %v", err)
	}
	if len(page) != 1 || page[0].ID != "r3" || after != "" {
		t.Fatalf("paginateResults(page2) = %+v, after=%q", page, after)
	}
}
