...
func resultSourceAndArgs(name string, meta executorMetadata) ([]string, []string) {
	if meta.QuerySource == querySourceHTTPHandler {
		...
		case queryChatPageUI:
			return []string{"internal/handlers/message.go ListMessages"}, []string{"bot_id", "session_id", "limit", "format=ui"}
		case queryLatestPage:
			return []string{"internal/handlers/message.go ListMessages"}, []string{"bot_id", "session_id", "limit", "format"}
		case queryBeforePage:
			return []string{"internal/handlers/message.go ListMessages"}, []string{"bot_id", "session_id", "limit", "format", "before_message_id"}
		case queryLocateWindow, queryExternalLookup:
			return []string{"internal/handlers/message.go LocateMessage"}, []string{"bot_id", "session_id", "external_message_id", "before", "after"}
		default:
...
func compositeResultSourceAndArgs(name string) ([]string, []string, bool) {
	...
	case queryLocateWindow:
		return []string{
			...
			"db/postgres/queries/tool_approval.sql ListToolApprovalsBySessionToolCalls",
			"db/postgres/queries/user_input.sql ListUserInputsBySessionToolCalls",
		}, []string{"session_id", "external_message_id", "cursor_message_id", "max_count", "message_ids", "bot_id", "tool_call_ids"}, true
