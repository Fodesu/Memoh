...
- `write_tool_tail`: hot agent-tool write path for `user -> assistant(tool-call) -> tool result -> assistant final`. It covers turn creation, request-bound assistant binding, and batched request-turn tail append for tool/final assistant messages.
