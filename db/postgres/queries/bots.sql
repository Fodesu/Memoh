-- name: CreateBot :one
INSERT INTO bots (owner_user_id, name, display_name, avatar_url, timezone, is_active, metadata, status, create_request_id)
...
-- name: GetBotByCreateRequest :one
-- The bot a previous POST /bots with this idempotency key created for the owner.
...
FROM bots
WHERE team_id = public.memoh_current_team_id() AND owner_user_id = $1 AND create_request_id = $2;
