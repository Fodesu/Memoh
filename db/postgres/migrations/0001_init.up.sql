...
-- orchestration_env_sessions: a concrete leased instance of an
-- env_resource. lease_epoch fences out stale writers in the same way
-- attempt claim_epoch does for task attempts.
...
-- orchestration_env_lease_reservations: admission queue for a resource
-- when capacity is saturated. Each reservation either commits to a
-- session row or aborts/expires.
...
-- orchestration_env_bindings: which task/attempt is currently using a
-- session. The binding can outlive a single attempt when status='held'
-- (HITL pause keeping the env warm for resume).
...
-- orchestration_env_snapshots: point-in-time captures keyed by session
-- so verifier replay and drift detection can compare runtime state
-- before/after an action without re-observing live.
