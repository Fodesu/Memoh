...
-- Durable session input queues. These are separate tables by design: a steer
-- can only target its current run, while a follow-up is assigned at a final
-- boundary to one continuation run.
...
CREATE INDEX IF NOT EXISTS session_steer_queue_pending_order ON public.session_steer_queue(team_id, session_id, position, item_id) WHERE status = 'accepted';
...
CREATE INDEX IF NOT EXISTS session_steer_queue_target_pending ON public.session_steer_queue(team_id, target_run_id, position, item_id) WHERE status = 'accepted';
CREATE INDEX IF NOT EXISTS session_steer_queue_claimed_run ON public.session_steer_queue(team_id, claim_run_id, created_at, item_id) WHERE status = 'claimed';
...
CREATE INDEX IF NOT EXISTS session_follow_up_queue_pending_order ON public.session_follow_up_queue(team_id, session_id, position, item_id) WHERE status = 'accepted' AND assigned_run_id IS NULL;
...
CREATE INDEX IF NOT EXISTS session_follow_up_queue_claimed_run ON public.session_follow_up_queue(team_id, claim_run_id, created_at, item_id) WHERE status = 'claimed';
CREATE INDEX IF NOT EXISTS session_follow_up_queue_assigned_active ON public.session_follow_up_queue(team_id, assigned_run_id, item_id) WHERE status IN ('accepted', 'claimed') AND assigned_run_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS session_follow_up_queue_enqueued_active ON public.session_follow_up_queue(team_id, enqueued_during_run_id, item_id) WHERE status IN ('accepted', 'claimed') AND assigned_run_id IS NULL;
CREATE INDEX IF NOT EXISTS session_follow_up_queue_enqueued_order ON public.session_follow_up_queue(team_id, enqueued_during_run_id, position, item_id);
