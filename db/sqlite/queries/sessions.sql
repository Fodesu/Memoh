...
UPDATE bot_sessions
SET head_turn_id = sqlc.narg(head_turn_id),
...
WHERE id = sqlc.arg(id)
  AND deleted_at IS NULL
  AND (
    head_turn_id = sqlc.narg(expected_head_turn_id)
    OR (head_turn_id IS NULL AND sqlc.narg(expected_head_turn_id) IS NULL)
  )
RETURNING *;
