...
- Tools are implemented as `ToolProvider` instances in `internal/agent/tool/`, loaded via setter injection to avoid FX dependency cycles. A tool is a `toolexec.Tool`; its `Execute` takes `sdk.ToolArguments` and returns `sdk.ToolOutput`. Map-based handlers decode with `inputAsMap` and type their result with `toolexec.OutputFromValue` / `toolexec.OutputPair`; map-literal schemas go through `toolexec.SchemaFromValue`. New tools should decode into a struct (`toolexec.NewTool[T]` infers the schema).
